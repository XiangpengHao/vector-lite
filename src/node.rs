...
impl<const N: usize> Node<N> {
    ...
    pub(crate) fn build_tree<R: Rng>(
        ...
        let node_above = Self::build_tree(max_leaf_size, &above, all_vectors, rng);
        let node_below = Self::build_tree(max_leaf_size, &below, all_vectors, rng);
        ...
        match self {
            Node::Leaf(leaf) => {
                ...
                if leaf.0.len() > max_leaf_size {
                    let (hyperplane, above, below) = build_hyperplane(&leaf.0, all_vectors, rng);
                    let node_above = Self::build_tree(max_leaf_size, &above, all_vectors, rng);
                    let node_below = Self::build_tree(max_leaf_size, &below, all_vectors, rng);
            ...
            Node::Inner(inner) => {
                let is_above = inner
                    .hyperplane
                    .point_is_above(&all_vectors[vector_idx as usize]);

                match is_above {
                    true => inner
                        .above
                        .insert(all_vectors, vector_idx, rng, max_leaf_size),
                    false => inner
                        .below
                        .insert(all_vectors, vector_idx, rng, max_leaf_size),
                }
    ...
    pub(crate) fn delete(&mut self, all_vectors: &[Vector<N>], vector_idx: u32) -> bool {
        match self {
            Node::Leaf(leaf) => {
            ...
            Node::Inner(inner) => {
                ...
                match is_above {
                    true => {
                        let needs_merge = inner.above.delete(all_vectors, vector_idx);
                        if needs_merge {
                    ...
                    false => {
                        let needs_merge = inner.below.delete(all_vectors, vector_idx);
                        if needs_merge {
                            *self = std::mem::replace(
                                &mut inner.above,
                                Node::Leaf(Box::new(LeafNode(Vec::new()))),
...
mod tests {
    ...
    fn test_insert_and_search_basic() {
        ...
        let indexes = vec![0, 1];
        let mut node = Node::build_tree(max_leaf_size, &indexes, &vectors, &mut rng);

        // Insert remaining vectors one by one
        for i in 2..vectors.len() {
            node.insert(&vectors, i as u32, &mut rng, max_leaf_size);
        }
    ...
    fn test_search_approximate_nearest() {
        ...
        let node = Node::build_tree(max_leaf_size, &indexes, &vectors, &mut rng);
    ...
    fn test_incremental_insertion() {
        ...
        node.insert(&vectors, 1, &mut rng, max_leaf_size);
    ...
    fn test_delete_operations() {
        // Test 1: Basic leaf node deletion
        {
            ...
            let result = node.delete(&vectors, first_idx);

            assert_eq!(result, false);

            let second_idx = vector_indexes[1];
            let result = node.delete(&vectors, second_idx);
        ...
        // Test 2: Node collapse when deleting from inner node
        {
            ...
            let result = node.delete(&vectors, 1); // Delete the "above" vector
        ...
        // Test 3: Cascading collapse in multi-level tree
        {
            ...
            node.delete(&vectors, 2); // Delete a vector from right side
            node.delete(&vectors, 3); // Delete the other vector from right side

            let after_delete_count = node.inner_node_count();
            assert!(after_delete_count < initial_inner_count);

            node.delete(&vectors, 0);
            let final_result = node.delete(&vectors, 1);
